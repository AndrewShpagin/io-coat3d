...
def RemoveFbxNodes(objekti):
    ...
    Prin_mat.inputs[6].default_value = 0
    Prin_mat.inputs[7].default_value = 0.29
    Prin_mat.inputs[0].default_value = 0.5, 0.5, 0.5, 1.0
    ...
    for ind, index_mat in enumerate(objekti.material_slots):
        ...
        if(udim_textures == False):
            for slot_index, texture_info in enumerate(texturelist):
                ...
                xprint(texture_info)
                xprint(index_mat)
                real_name = index_mat.name
                L = len(real_name)
                if(L > 4 and real_name[L - 4] == '.'):
                    ends = real_name[L - 3:]
                    if ends.isdigit():
                        real_name = real_name[:L - 4]
                        xprint('Name changed', index_mat.name, real_name)
                if(texture_info[0] == real_name):
                    ...
                    elif texture_info[2] == 'gloss' or texture_info[2] == 'glossness':
                        if (index_mat.material.coat3D_roughness):
                            texcoat['gloss'].append(texture_info[3])
                            create_nodes = True
                        else:
                            os.remove(texture_info[3])
            ...
            xprint('texturelist', texturelist)
            for texture_info in texturelist:
                xprint('texture_info', texture_info)
                if texture_info[2] == 'color' or texture_info[2] == 'diffuse':
...
def createnodes(active_mat,texcoat, create_group_node, tile_list, objekti, ind, is_new): # Creates new nodes and link textures into them
    ...
    coatMat.show_transparent_back = False
    
    # First go through all image nodes and let's check if it starts with 3DC and reload if needed

    xprint('createnodes', texcoat)

    for node in coatMat.node_tree.nodes:        
        xprint('    node', node.name, node.type, len(node.inputs), len(node.outputs))
        if(node.name == 'Principled BSDF'):
            node.inputs[6].default_value = 0
            node.inputs[7].default_value = 0.29
            node.inputs[0].default_value = 0.5, 0.5, 0.5, 1.0
        if (node.type == 'OUTPUT_MATERIAL'):
            # node.inputs[0].default_value = 0.5, 0.5, 0.7
            out_mat = node
            break
    ...
    if(create_group_node):
        if(applink_group_node == False):
            ...
            group_tree.interface.new_socket(name="Color", socket_type="NodeSocketColor", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketColor", "Metallic")
            group_tree.interface.new_socket(name="Metallic", socket_type="NodeSocketColor", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketColor", "Roughness")
            group_tree.interface.new_socket(name="Roughness", socket_type="NodeSocketColor", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketVector", "Normal map")
            group_tree.interface.new_socket(name="Normal map", socket_type="NodeSocketVector", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketColor", "Emissive")
            group_tree.interface.new_socket(name="Emissive", socket_type="NodeSocketColor", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketColor", "Displacement")
            group_tree.interface.new_socket(name="Displacement", socket_type="NodeSocketColor", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketColor", "Emissive Power")
            group_tree.interface.new_socket(name="Emissive Power", socket_type="NodeSocketColor", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketColor", "AO")
            group_tree.interface.new_socket(name="AO", socket_type="NodeSocketColor", in_out='OUTPUT')
            # group_tree.outputs.new("NodeSocketColor", "Alpha")
            group_tree.interface.new_socket(name="Alpha", socket_type="NodeSocketColor", in_out='OUTPUT')
        ...
        json_address = os.path.dirname(bpy.app.binary_path) + os.sep + str(bpy.app.version[0]) + '.' + str(bpy.app.version[1]) + os.sep + 'scripts' + os.sep + 'addons_core' + os.sep + 'io_coat3D' + os.sep + 'data.json'
        ...
        if(out_mat.inputs['Surface'].is_linked == True):
            ...
            if(bring_roughness == True and texcoat['gloss'] != []):
                CreateTextureLine(data['rough'], act_material, main_mat, texcoat, coat3D, notegroup,
                                  main_material, applink_tree, out_mat, coatMat,tile_list, objekti, ind, is_new)
...
def get_params_from_name(name):
    ...
    if idx>0:
        js = name[idx + 1:]
        while len(js) > 0 and js[-1] != '}':
            js = js[:-1]
        new_name = new_name.replace(' &' + js, '')
        xprint('json:', js)
        mat_params = json.loads(js)
    return mat_params
def CreateTextureLine(type, act_material, main_mat, texcoat, coat3D, notegroup, main_material, applink_tree, out_mat, coatMat, tile_list, objekti, ind, is_new):
    xprint('===================================================================================\nCreateTextureLine');
    xprint('type', type)
    xprint('act_material', act_material)
    xprint('main_mat', main_mat)
    xprint('texcoat', texcoat)
    xprint('coat3D', coat3D)
    xprint('notegroup', notegroup)
    xprint('main_material', main_material)
    xprint('out_mat', out_mat)
    xprint('coatMat', coatMat)
    xprint('ind', ind)
    xprint('coatMat.name', coatMat.name)

    mat_params = get_params_from_name(coatMat.name)
    
    tname = type['name']
    invert = False
    if texcoat[tname] == [] and tname == 'rough':
        tname = 'gloss'
        invert = True
    if(tile_list):
        texture_name = coatMat.name + '_' + tname
        ...
        notegroupend = texture_tCree.nodes.new('NodeGroupOutput')
        ...
        for index, tile in enumerate(tile_list):
            ...
            for ind, tex_index in enumerate(texcoat[tname]):
                if(tex_index[0] == tile):
                    tex_img_node.image = bpy.data.images.load(texcoat[tname][ind][1])
                    break
            tex_img_node.location = tex_loc

            if tex_img_node.image and type['colorspace'] != 'color' and tname != 'color':
    ...
    if tname == 'nmap':
        ...
        normal_node.location = -50, -350
        normal_node.name = '3DC_normalnode'

        invRG = act_material.nodes.new('ShaderNodeRGBCurve')
        curvR = invRG.mapping.curves[0]
        if ('B' in mat_params):
            curvR.points[0].location = 0,1
            curvR.points[1].location = 1,0
        
        curvG = invRG.mapping.curves[1]
        if ('T' in mat_params):
            curvG.points[0].location = 0,1
            curvG.points[1].location = 1,0

        bump = 2.0
        if 'bm' in mat_params:
            bump = mat_params['bm'] * 2.0

        normal_node.inputs[0].default_value = bump
        invRG.location = -350, -350


    elif tname == 'displacement':
    ...
    node.name = '3DC_' + tname
    node.label = tname

    if (tname != 'displacement'):
    ...
    if (tile_list == []):
        ...
        for image in bpy.data.images:
            if(texcoat[tname][0] == image.filepath):
                load_image = False
                node.image = image
        ...
        if (load_image):
            node.image = bpy.data.images.load(texcoat[tname][0])
            node.image.colorspace_settings.name = "sRGB"
    ...
    if (coat3D.createnodes):
        ...
        if(tname == 'nmap'):
            act_material.links.new(node.outputs[0], invRG.inputs[1])
            act_material.links.new(invRG.outputs[0], normal_node.inputs[1])
            if(input_color != -1):
            ...
            if (texcoat['alpha'] != []):
                ...
                if (tname == 'color'):
                    act_material.links.new(node.outputs[1], notegroup.inputs[8])
            if(tname != 'alpha'):
                huenode = createExtraNodes(act_material, node, type, notegroup, invert, mat_params)
            else:
                huenode = node
            ...
            if (main_mat.type != 'MIX_SHADER' and input_color != -1):
                main_material.links.new(applink_tree.outputs[type['input']], main_mat.inputs[input_color])
                if(tname == 'color'): #Alpha connection into Principled shader
...
def createExtraNodes(act_material, node, type, notegroup, invert, mat_params):
    mod_value= 1.0    
    if(type['name'] == 'rough'):
        if('rg' in mat_params):
            mod_value = mat_params['rg']
        if('gl' in mat_params):
            mod_value = mat_params['gl']
            invert = True
    if(type['name'] == 'metalness'):
        if('mt' in mat_params):
            mod_value = mat_params['mt']

    xprint('createExtraNodes:', type['name'], mod_value)
    if(type['huenode'] == 'yes'):
        ...
        huenode.name = '3DC_Mod'
        if invert:
            huenode.operation = 'MULTIPLY_ADD'
            huenode.inputs[1].default_value = -mod_value
            huenode.inputs[2].default_value = 1
            huenode.use_custom_color = True
            huenode.color = (type['node_color'][0], type['node_color'][1], type['node_color'][2])
        else:
    ...
    if (type['rampnode'] == 'yes'):
        ...
        if (type['huenode'] == 'yes'):
            act_material.links.new(node.outputs[0], huenode.inputs[4])
        else:
            act_material.links.new(node.outputs[0], huenode.inputs[0])

    if type['name'] == 'metalness':
        node.location = -1000, 119
    ...
    if(type['name'] == 'color'):
        ...
        node_mixRGB.location = -100, 500
        node_vectormath.location = 100,400
        
        # additional alpha tweak, by Andrew
        xn = -1300
        yn =  -470-400
        stp = 200

        node_vertex.location = xn, yn
        xn += stp

        add1 = act_material.nodes.new('ShaderNodeMath')
        add1.name = '3DC_alpha_add1'
        add1.operation = 'ADD'
        add1.inputs[1].default_value = 0.001
        add1.location = xn, yn
        xn += stp
        act_material.links.new(node_vertex.outputs[1], add1.inputs[0])
        

        div1 = act_material.nodes.new('ShaderNodeMath')
        div1.name = '3DC_alpha_div1'
        div1.operation = 'DIVIDE'        
        div1.location = xn, yn
        xn += stp
        act_material.links.new(node_vertex.outputs[1], div1.inputs[0])
        act_material.links.new(add1.outputs[0], div1.inputs[1])

        sub1 = act_material.nodes.new('ShaderNodeMath')
        sub1.name = '3DC_alpha_sub1'
        sub1.operation = 'SUBTRACT'     
        sub1.inputs[0].default_value = 1.01
        sub1.location = xn, yn
        xn += stp
        act_material.links.new(div1.outputs[0], sub1.inputs[1])  
        
        pow1 = act_material.nodes.new('ShaderNodeMath')
        pow1.name = '3DC_alpha_pow1'
        pow1.operation = 'POWER'        
        pow1.location = xn, yn
        xn += stp
        act_material.links.new(node_vertex.outputs[1], pow1.inputs[1])
        act_material.links.new(sub1.outputs[0], pow1.inputs[0])

        sub2 = act_material.nodes.new('ShaderNodeMath')
        sub2.name = '3DC_alpha_sub2'
        sub2.operation = 'SUBTRACT'     
        sub2.inputs[0].default_value = 1
        sub2.location = xn, yn
        xn += stp
        act_material.links.new(pow1.outputs[0], sub2.inputs[1])

        mul1 = act_material.nodes.new('ShaderNodeMath')
        mul1.name = '3DC_alpha_mul1'
        mul1.operation = 'MULTIPLY'     
        mul1.location = xn, yn
        xn += stp
        act_material.links.new(node.outputs[1], mul1.inputs[0])
        act_material.links.new(sub2.outputs[0], mul1.inputs[1])

        act_material.links.new(mul1.outputs[0], notegroup.inputs[8])
        
        act_material.links.new(node_vertex.outputs[0], node_mixRGB.inputs[2])
        act_material.links.new(huenode.outputs[0], node_mixRGB.inputs[1])
...
def matlab(objekti,mat_list,texturelist,is_new):
    ...
    xprint('Welcome facture matlab function')
    xprint('mat_list', mat_list)
    xprint('texturelist', texturelist)
