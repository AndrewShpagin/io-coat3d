...
bl_info = {
    ...
    "author": "Kalle-Samuli Riihikoski (haikalle) and Andrew Shpagin",
    "version": (5, 0, 0),
    "blender": (4, 2, 0),
...
def every_3_seconds():
    ...
    try:
        ...
        Export_folder_Blender  = coat3D.exchangeFolder
        Export_folder_Blender += os.sep + 'Blender' + os.sep + 'export.txt'
        if (os.path.isfile(Export_folder_Blender) and mTime != os.path.getmtime(Export_folder_Blender)):
            xprint('Found object:', Export_folder_Blender)
            mTime = os.path.getmtime(Export_folder_Blender)
            workflow2(coat3D.exchangeFolder + os.sep + 'Blender' + os.sep)
...
class SCENE_OT_getback(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        xprint("SCENE_OT_getback::invoke")
        global global_exchange_folder
        path_ex = ''
        ...
        if (bpy.app.background == False):
            if os.path.isfile(Export_folder):
                ...
                xprint('BLENDER -> 3DC -> BLENDER WORKFLLOW')
                DeleteExtra3DC()
...
class SCENE_OT_opencoat(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        xprint("SCENE_OT_opencoat::invoke")
...
class SCENE_OT_export(bpy.types.Operator):
    ...
    def invoke(self, context, event):
        xprint("SCENE_OT_export::invoke")
        bpy.ops.export_applink.pilgway_3d_coat()

        return {'FINISHED'}

    def execute(self, context):
        xprint("SCENE_OT_export::execute")
        global foundExchangeFolder
        global global_exchange_folder
        ...
        if(coat3D.type == 'autopo'):
            ...
            coat3D.bring_retopo = False
            bpy.ops.export_scene.fbx(filepath=coa.applink_address,global_scale = 1, use_selection=True, use_mesh_modifiers=coat3D.exportmod, axis_forward='-Z', axis_up='Y')
...
def blender_3DC(texturelist, new_applink_address):
    ...
    for c_index0 in diff_objects:
        if c_index0.startswith('vt_'):
            facture_object = True
        if(facture_object):
            break
        for c_index in diff_objects:
            #xprint('c_index', c_index, '=?=', 'vt_' + c_index0)
            bpy.data.objects[c_index].data.coat3D.name = '3DC'
            laskuri += 1
            #xprint(laskuri)
            #xprint(diff_objects[0])
            if(c_index == ('vt_' + c_index0)):
                facture_object = True
                #print('Facture object found!!')
                break
    ...
    for obj in bpy.data.objects:
        for num, m in list(enumerate(obj.material_slots)):
            if m.material:
                ...
                xprint('material:', myhold)
                idx = myhold.find(' &{')
                new_name = myhold
                if idx>0:
                    xprint('found:', myhold)
                    m.material.name = myhold[:idx]
                    xprint ('Renamed:', myhold, "-->", m.material.name)
